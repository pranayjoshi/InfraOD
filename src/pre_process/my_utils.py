...
def seq_to_int(sequences:list) ->list:
    
    # Filter out non-sequence files and .DS_Store
    sequence_files = [seq for seq in sequences if seq.startswith("Sequence-") and seq != ".DS_Store"]
    return sorted(list(map(lambda x: int(x.split("-")[1]), sequence_files)))